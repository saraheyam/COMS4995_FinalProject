...
K562_peaks <- readPeakFile('ENCFF316KZZ.bed', as = "GRanges")
GM_peaks <- readPeakFile('GSE47753_GM12878_ATACseq_50k_AllReps_ZINBA_pp08.bed', as = "GRanges")
...
```{r}
as.data.frame(K562_annot)
```

```{r}
# process annotations to have original peak label for matching
get_annot <- function(annot_df) {
  df <- as.data.frame(annot_df)[, c("ENSEMBL", "seqnames", "start", "end", "annotation", "geneStart", "geneEnd")]
...
```{r}
#### MAKE K562_annots NOT A CONCATED LIST
#### DO THIS THING
### DO IT.
## DO IT MOTHERFUCKER


K562_annots$annotation <- gsub("Exon.*", "Exon", K562_annots$annotation) %>% 
  gsub("Intron.*", "Intron", .) 

GM_annots$annotation <- gsub("Exon.*", "Exon", GM_annots$annotation) %>% 
  gsub("Intron.*", "Intron", .) 
...
```{r}
K562_annots <- na.omit(K562_annots[c("peaks", "annotation", "geneStart", "geneEnd", "ENSEMBL")])
GM_annots <- na.omit(GM_annots[c("peaks", "annotation", "geneStart", "geneEnd", "ENSEMBL")])
...
```{r}
write.csv(GM_annots, "GM_ATAC_annots_bulk.csv", col.names = T)
write.csv(K562_annots, "K562_ATAC_annots_bulk.csv", col.names = T)
...
write.csv(GM_counts, "GM_ATAC_counts_bulk.csv")
write.csv(K562_counts, "K562_ATAC_counts_bulk.csv")
#write.csv(H1_counts, "H1_ATAC_counts.csv")
```

```{r}
# GO BACK TO THE EDA
# REPROCESS THE DATA FOR INPUT
# APPLY CSV DATA
# RUN IN TRAIN V3
```
